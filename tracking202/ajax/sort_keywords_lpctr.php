...
//only create a new list if there is no post
	if (($_POST['order']== '') and ($_POST['offset'] == '')) {
		...
		//delete old keyword list
			$mysql['user_id'] = mysql_real_escape_string($_SESSION['user_id']);
			$keyword_sql = "DELETE FROM `202_sort_keywords_lpctr` WHERE `user_id`='".$mysql['user_id']."'";
...
// Clean escape characters from strings
					$mysql['sort_keyword_click_throughs'] = mysql_real_escape_string($total_click_throughs);
					$mysql['sort_keyword_ctr'] = mysql_real_escape_string($total_ctr_ratio);
				...
				//insert the data
					$keyword_sort_sql = "INSERT INTO 202_sort_keywords_lpctr
										 SET         user_id='".$mysql['user_id']."',
													 keyword_id='".$mysql['keyword_id']."',
													 sort_keyword_clicks='".$mysql['sort_keyword_clicks']."',

													 sort_keyword_click_throughs='".$mysql['sort_keyword_click_throughs']."',
													 sort_keyword_ctr='".$mysql['sort_keyword_ctr']."',
...
$html['sort_keyword_click_throughs_order'] = 'sort_keyword_click_throughs asc';
if ($_POST['order'] == 'sort_keyword_click_throughs asc') {
	$html['sort_keyword_click_throughs_order'] = 'sort_keyword_click_throughs desc';
	$mysql['order'] = 'ORDER BY `sort_keyword_click_throughs` DESC';
} elseif ($_POST['order'] == 'sort_keyword_click_throughs desc') {
	$html['sort_keyword_click_throughs_order'] = 'sort_keyword_click_throughs asc';
	$mysql['order'] = 'ORDER BY `sort_keyword_click_throughs` ASC';
}

$html['sort_keyword_ctr_order'] = 'sort_keyword_ctr asc';
if ($_POST['order'] == 'sort_keyword_ctr asc') {
	$html['sort_keyword_ctr_order'] = 'sort_keyword_ctr desc';
	$mysql['order'] = 'ORDER BY `sort_keyword_ctr` DESC';
} elseif ($_POST['order'] == 'sort_keyword_ctr desc') {
	$html['sort_keyword_ctr_order'] = 'sort_keyword_ctr asc';
	$mysql['order'] = 'ORDER BY `sort_keyword_ctr` ASC';
}
...
<table cellspacing="0" cellpadding="0" style="width: 100%; font-size: 12px;">
	<tr>
		<td width="100%;">
			<a target="_new" href="/tracking202/analyze/keywords_lpctr_download.php">
...
<table cellpadding="0" cellspacing="1" class="m-stats">
	<tr>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_keywords_lpctr.php','','<? echo $html['sort_keyword_keyword_order']; ?>');">Keyword</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_keywords_lpctr.php','','<? echo $html['sort_keyword_clicks_order']; ?>');">Clicks</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_keywords_lpctr.php','','<? echo $html['sort_keyword_click_throughs_order']; ?>');">Click Throughs</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_keywords_lpctr.php','','<? echo $html['sort_keyword_ctr_order']; ?>');">LP CTR</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_keywords_lpctr.php','','<? echo $html['sort_keyword_leads_order']; ?>');">Leads</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_keywords_lpctr.php','','<? echo $html['sort_keyword_su_ratio_order']; ?>');">S/U</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_keywords_lpctr.php','','<? echo $html['sort_keyword_payout_order']; ?>');">Payout</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_keywords_lpctr.php','','<? echo $html['sort_keyword_epc_order']; ?>');">EPC</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_keywords_lpctr.php','','<? echo $html['sort_keyword_avg_cpc_order']; ?>');">Avg CPC</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_keywords_lpctr.php','','<? echo $html['sort_keyword_income_order']; ?>');">Income</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_keywords_lpctr.php','','<? echo $html['sort_keyword_cost_order']; ?>');">Cost</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_keywords_lpctr.php','','<? echo $html['sort_keyword_net_order']; ?>');">Net</a></th>
		<th><a class="onclick_color" onclick="loadContent('/tracking202/ajax/sort_keywords_lpctr.php','','<? echo $html['sort_keyword_roi_order']; ?>');">ROI</a></th>
	</tr>
...
while ($keyword_row = mysql_fetch_array($keyword_result, MYSQL_ASSOC)) {
	...
	$html['sort_keyword_click_throughs'] = htmlentities($keyword_row['sort_keyword_click_throughs'], ENT_QUOTES, 'UTF-8');
	$html['sort_keyword_ctr'] = htmlentities($keyword_row['sort_keyword_ctr'], ENT_QUOTES, 'UTF-8');
	...
	<tr>
		...
		<td class="m-row1"><? echo $html['sort_keyword_click_throughs']; ?></td>
		<td class="m-row1"><? echo $html['sort_keyword_ctr']; ?></td>
...
<? if ($query['pages'] > 2) { ?>
	<div class="offset">   <?
		if ($query['offset'] > 0) {
			printf(' <a class="onclick_color" onclick="loadContent(\'/tracking202/ajax/sort_keywords_lpctr.php\',\'%s\',\'%s\');">First</a> ', $i, $html['order']);
			printf(' <a class="onclick_color" onclick="loadContent(\'/tracking202/ajax/sort_keywords_lpctr.php\',\'%s\',\'%s\');">Prev</a> ', $query['offset'] - 1, $html['order']);
		}
		...
		if ($query['pages'] > 1) {
			for ($i=0; $i < $query['pages']-1; $i++) {
				if (($i >= $query['offset'] - 10) and ($i < $query['offset'] + 11)) {
					...
					printf(' <a %s onclick="loadContent(\'/tracking202/ajax/sort_keywords_lpctr.php\',\'%s\',\'%s\');">%s</a> ', $class, $i, $html['order'], $i+1);
					unset($class);
				}
			}
		}
		...
		if ($query['offset'] < $query['pages'] - 2) {
			printf(' <a class="onclick_color" onclick="loadContent(\'/tracking202/ajax/sort_keywords_lpctr.php\',\'%s\',\'%s\');"">Next</a> ', $query['offset'] + 1, $html['order']);
			printf(' <a class="onclick_color" onclick="loadContent(\'/tracking202/ajax/sort_keywords_lpctr.php\',\'%s\',\'%s\');">Last</a> ', $query['pages'] - 2, $html['order']);
		} ?>
	</div>
<? } ?>
